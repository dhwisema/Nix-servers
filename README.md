...
0. Make sure you are able to access root ssh of host system ie in the nixos installer or in prior os
 1. Create a new folder for the new host. Create Configuration.nix and disk-config.nix. Leaving hardware-configuration.nix alone for now. 
 1.1. Also copy disk-config.nix to the generic host
 1.2 add new host to flake.nix
 2. Run on either laptop or desktop:
`cd ~/Nix-servers && nix run github:nix-community/nixos-anywhere -- --generate-hardware-config nixos-generate-config ./hardware-configuration.nix --flake ./#generic --target-host root@<ip address>
`
1. If no errors found during install yay we installed nix to a new host. 
2. Post install things to do
